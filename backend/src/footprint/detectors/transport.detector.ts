...
export const detectTransport: ActivityDetector = (text) => {
  ...
  let busKm = sumKm(
    text,
    /(\d+(?:[.,]\d+)?)\s*km\s*(?:en\s*)?(?:bus|autobús|autobus|colectivo|transporte\s*público)/gi,
  );
  if (busKm === 0) busKm = sumKm(text, /(?:tomé|tomar|agarré|tomando)\s*(?:el\s*)?(?:bus|autobús|autobus|colectivo)\s*[^\d]*?(\d+(?:[.,]\d+)?)\s*km/gi);
  if (busKm === 0) busKm = sumKm(text, /(\d+(?:[.,]\d+)?)\s*km\s*(?:en\s*)?(?:tomando\s*)?(?:el\s*)?(?:bus|autobús|autobus)/gi);
  if (busKm > 0) {
    items.push({
  ...
  let carKm = sumKm(
    text,
    /(\d+(?:[.,]\d+)?)\s*km\s*(?:en\s*)?(?:carro|coche|auto|automóvil|automovil|camión|camion)/gi,
  );
  if (carKm === 0) carKm = sumKm(text, /(?:manejé|manejar|manejo|conduje|conducir|manejando)\s*[^\d]*?(\d+(?:[.,]\d+)?)\s*km/gi);
  if (carKm === 0) carKm = sumKm(text, /(\d+(?:[.,]\d+)?)\s*km\s*(?:manejando|en\s*carro)/gi);
  if (carKm > 0) {
    items.push({
  ...
  if (planeKm === 0) planeKm = sumKm(text, /(?:avión|avion|vuelo)\s*[^\d]*?(\d+(?:[.,]\d+)?)\s*km/gi);
  if (planeKm === 0) planeKm = sumKm(text, /(?:volé|volar|volamos|volando)\s*[^\d]*?(\d+(?:[.,]\d+)?)\s*km/gi);
  if (planeKm === 0) planeKm = sumKm(text, /(\d+(?:[.,]\d+)?)\s*km\s*(?:volando|en\s*avión)/gi);
  if (planeKm > 0) {
    items.push({
  ...
  if (walkKm === 0) walkKm = sumKm(text, /(?:caminé|caminar)\s*(\d+(?:[.,]\d+)?)\s*km/gi);
